...
if __name__ == "__main__":
    ...
    if method == 'PIELM':
        ...
        best_pde_loss, best_coef_solut = trainer.train_auto(
            X=X_res, n_epoch=epoch, lr=1e-3, print_interval=10, patience=1000, update_interval=100)
    ...
    if method == 'PIELM':
        visual.plot_solutions_1D(X_plot, save_path="Save_Fig/noneLinear",filename="PIELM_Helmholtz1D.pdf")
    
    elif method == 'RINN':
        visual.plot_solutions_1D(X_plot, save_path="Save_Fig/noneLinear",filename="RINN_Helmholtz1D.pdf")

    elif method == 'RINN_es':
        visual.plot_solutions_1D(X_plot, save_path="Save_Fig/noneLinear",filename="RINN_es_Helmholtz1D.pdf")
    else:
