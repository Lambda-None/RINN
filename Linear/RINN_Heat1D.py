...
if __name__ == "__main__":
    ...
    print(f"Absolute Mean Error: {Abs_mean:.2e}")

    X_plot = X.cpu()
    T_plot = T.cpu()
    u_test = u_test.detach().cpu().numpy().reshape(X.shape)
    u_true = u_true.detach().cpu().numpy().reshape(X.shape)
    error = np.abs(u_test - u_true)

    visual = visualization(u_test, u_true, k)

    if method == 'PIELM':
        visual.plot_solutions_2D(X_plot, T_plot, save_path="Save_Fig/linear_evolution",filename="PIELM_Heat1D.pdf")
    
    elif method == 'RINN':
        visual.plot_solutions_2D(X_plot, T_plot, save_path="Save_Fig/linear_evolution",filename="RINN_Heat1D.pdf")

    elif method == 'RINN_es':
        visual.plot_solutions_2D(X_plot, T_plot, save_path="Save_Fig/linear_evolution",filename="RINN_es_Heat1D.pdf")
    else:
