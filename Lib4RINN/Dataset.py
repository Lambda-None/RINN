...
class BcsPoint:
    ...
    def hypersquare(domain, num_bdry_pts, dtype=torch.float32):
        n = len(domain) // 2

        d = domain.reshape(n, 2)
        mins = d[:, 0]
        maxs = d[:, 1]
        ranges = (maxs - mins)

        if n == 1:
            sides = torch.randint(0, 2, (num_bdry_pts,))
            vals = torch.where(sides == 0, mins[0], maxs[0]).to(dtype=dtype)
            return vals.unsqueeze(1) 
        
        face_ids = torch.randint(0, 2 * n, (num_bdry_pts,))  
        face_dims = (face_ids // 2).to(torch.long)   
        face_sides = (face_ids % 2).to(torch.long)   

        sobol = torch.quasirandom.SobolEngine(dimension=n - 1, scramble=True)
        samples = sobol.draw(num_bdry_pts).to(dtype=dtype)  

        X = torch.empty((num_bdry_pts, n), dtype=dtype)

        for dim in range(n):
            idx = (face_dims == dim).nonzero(as_tuple=True)[0]
            if idx.numel() == 0:
                continue
            other_dims = [i for i in range(n) if i != dim]
            rows = samples[idx, :]
            X[idx][:, other_dims] = rows * ranges[other_dims].unsqueeze(0) + mins[other_dims].unsqueeze(0)
            
            val_dim_min = mins[dim]
            val_dim_max = maxs[dim]
            sides = face_sides[idx]  
            X[idx, dim] = torch.where(sides == 0, val_dim_min, val_dim_max)

        X_bcs = X.to(dtype=dtype)
        X_ini = torch.empty([0, n], dtype=dtype)  # you must have this as a placeholder
        return X_ini, X_bcs
...
class ResPoint:
    ...
    def hypersquare(domain, num_colloc_pts, dtype=torch.float32):
        total = domain.numel()
        n = total // 2
        ...
        sobol = torch.quasirandom.SobolEngine(dimension=n, scramble=True)
        samples = sobol.draw(num_colloc_pts).to(dtype=dtype) 

        X_res = samples * ranges.unsqueeze(0) + mins.unsqueeze(0) 
        X_res = X_res.to(dtype=dtype)
