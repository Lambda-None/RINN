...
class Test:
    def __init__(self, domain, pde, NN, num_eval_pts=201, dtype=torch.float32, device='cpu'):        
        self.domain = domain
        self.pde = pde
        self.networks = NN
    ...
    def line(self):
        ...
        X_eval = torch.linspace(xmin, xmax, self.num_eval_pts, dtype=self.dtype, device=self.device)[:,None]
        return X_eval
    
    def rectangle(self):
        xmin, xmax, ymin, ymax = self.domain
        lin_x = torch.linspace(xmin, xmax, self.num_eval_pts, dtype=self.dtype, device=self.device)
        lin_y = torch.linspace(ymin, ymax, self.num_eval_pts, dtype=self.dtype, device=self.device)
    ...
    def Lshape(self):
        ...
        x_mid = (xmin + xmax) / 2
        y_mid = (ymin + ymax) / 2
        x_mid = torch.tensor(x_mid, dtype=self.dtype, device=self.device)
        y_mid = torch.tensor(y_mid, dtype=self.dtype, device=self.device)
    ...
    def cube(self):
        ...
        lin_x = torch.linspace(xmin, xmax, self.num_eval_pts, dtype=self.dtype, device=self.device)
        lin_y = torch.linspace(ymin, ymax, self.num_eval_pts, dtype=self.dtype, device=self.device)
        lin_z = torch.linspace(zmin, zmax, self.num_eval_pts, dtype=self.dtype, device=self.device)
    ...
    def hypersquare(self, num_eval_pts):
        n = len(self.domain) // 2
        domain = self.domain.view(n, 2)  

        # 在 [0,1]^n 内采样
        X_unit = torch.rand((num_eval_pts, n), dtype=self.dtype, device=self.device)

        # 映射到实际 domain
        lower = domain[:, 0]
        upper = domain[:, 1]
        X_eval = lower + (upper - lower) * X_unit
    ...
    def predict(self, X):
        self.networks.eval()    
        u_test = self.networks.forward(X)
        u_true = self.pde.exact(X)
        return u_test, u_true

    def error_compute(self, u_test, u_true):
        L2 = torch.sqrt(torch.mean((u_test - u_true) ** 2))
        Rele_L2 = L2 / torch.sqrt(torch.mean(u_true ** 2))
